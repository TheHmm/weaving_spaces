defmodule KnitMakerWeb.CoreComponents do
  @moduledoc """
  ...
  def modal(assigns) do
    ...
    >
      <div id={"#{@id}-bg"} class="fixed inset-0 bg-zinc-50/90" aria-hidden="true" />
      <div
        ...
        <div class="flex min-h-full items-center justify-center">
          <div class="w-full max-w-3xl p-4 sm:p-6 lg:py-8">
            <.focus_wrap
              ...
              class="hidden relative rounded-2xl bg-white p-14 shadow-lg shadow-zinc-700/10 ring-1 ring-zinc-700/10"
            >
    ...
    <div class="overflow-y-auto px-4 sm:overflow-visible sm:px-0">
      <table class="mt-11 w-[40rem] sm:w-full">
        ...
        <tbody
          ...
          phx-update={match?(%Phoenix.LiveView.LiveStream{}, @rows) && "stream"}
          phx-hook="Sortable"
          class="relative divide-y divide-zinc-100 border-t border-zinc-200 text-sm leading-6 text-zinc-700 select-none"
        >
          <tr
            :for={row <- @rows}
            id={@row_id && @row_id.(row)}
            class="group hover:bg-zinc-50 item drag-item:focus-within:ring-0 drag-item:focus-within:ring-offset-0 drag-ghost:bg-zinc-300 drag-ghost:border-0 drag-ghost:ring-0"
            data-id={@row_id && @row_id.(row)}
          >
            <td
              :for={{col, i} <- Enum.with_index(@col)}
              phx-click={@row_click && @row_click.(row)}
  ...
  def hide(js \\ %JS{}, selector) do
    JS.hide(js,
      ...
      time: 200,
      transition:
        {"transition-all transform ease-in duration-20", "opacity-20 translate-y-0 sm:scale-100",
         "opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95"}
    )
  end
  ...
  def show_modal(js \\ %JS{}, id) when is_binary(id) do
    ...
    |> JS.show(
      to: "##{id}-bg",
      transition: {"transition-all transform ease-out duration-20", "opacity-0", "opacity-100"}
    )
    |> show("##{id}-container")
  ...
  def hide_modal(js \\ %JS{}, id) do
    ...
    |> JS.hide(
      to: "##{id}-bg",
      transition: {"transition-all transform ease-in duration-20", "opacity-100", "opacity-0"}
    )
    |> hide("##{id}-container")
  ...
  def translate_error({msg, opts}) do
    ...
    #   Gettext.dngettext(KnitMakerWeb.Gettext, "errors", msg, msg, count, opts)
    # else
    #   Gettext.dgettext(KnitMakerWeb.Gettext, "errors", msg, opts)
    # end
